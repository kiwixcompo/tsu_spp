...
return [
    ...
    'env' => ($_SERVER['HTTP_HOST'] ?? '') === 'staff.tsuniversity.edu.ng' ? 'production' : ($_ENV['APP_ENV'] ?? 'local'),
    'debug' => ($_SERVER['HTTP_HOST'] ?? '') !== 'staff.tsuniversity.edu.ng' && filter_var($_ENV['APP_DEBUG'] ?? true, FILTER_VALIDATE_BOOLEAN),
    'url' => ($_SERVER['HTTP_HOST'] ?? '') === 'staff.tsuniversity.edu.ng' ? 'https://staff.tsuniversity.edu.ng' : ($_ENV['APP_URL'] ?? 'http://localhost/tsu_spp/public'),
